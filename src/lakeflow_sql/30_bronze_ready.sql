-- Trusted Source Intake — Certified ready output
-- Only rows that pass all 7 contract checks and are not replay duplicates
-- reach this view. Downstream consumers should read from here, not from
-- bronze_orders_raw.

CREATE OR REFRESH MATERIALIZED VIEW bronze_ready
COMMENT 'Contract-compliant rows certified for downstream consumption'
AS
